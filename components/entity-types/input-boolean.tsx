...
export default function EntityInputBoolean({ allEntities }) {
  ...
  const turnOn = () => {
    setState(true);
    callService(connection, 'input_boolean', 'turn_on', { entity_id });
  };

  const turnOff = () => {
    setState(false);
    callService(connection, 'input_boolean', 'turn_off', { entity_id });
  };
