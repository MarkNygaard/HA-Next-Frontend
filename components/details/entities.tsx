...
export default function DetailsEntity({
  allEntities,
}) {

  const { connection } = useHassStore();

    const entity = useEntity(allEntities.entity_id);
    ...
    const turnOn = () => {
      setState('on');
      callService(connection, 'light', 'turn_on', {allEntities.entity_id});
    };

    const turnOff = () => {
      setState('off');
      callService(connection, 'light', 'turn_off', {allEntities.entity_id});
    };
    ...
    return (
          <button
            className="m-1 flex w-2/3 justify-center bg-blue-500 p-2 text-xl font-normal text-white dark:text-black"
            onClick={toggle}
            type="button"
          >
            <div className="flex justify-center px-4 text-base">
              {allEntities.entity_name}
            </div>
